...
{
    public GameObject PointPrefab; //생성할 발판의 원본 프리팹
    ...
    private GameObject[] Points; //미리 생성한 발판들
    ...
    void Start()
    {
        ...
        {
            Points[i] = Instantiate(PointPrefab, poolPosition, Quaternion.identity);// identity
        }
        lastSpwnTime = 0f;
        timeBetSpawn = 0f;
    }
    ...
    void Update()
    {
        ...
        if (Time.time >= lastSpwnTime + timeBetSpawn)
        {
            ...
            Points[currentIndex].SetActive(false);
            Points[currentIndex].SetActive(true);

            Points[currentIndex].transform.position = new Vector2(xPos, yPos);
            currentIndex++;
